...
public class APIVaccineVisitController extends APIController {
    ...
    public ResponseEntity createVaccineVisit ( @RequestBody final VaccineVisitForm vvf ) {
        try {
            ...
            if ( request.getCptCodes() != null && request.getCptCodes().size() > 0 ) {
                final List<CPTCode> l = request.getCptCodes();
                int sum = 0;
                for ( final CPTCode c : l ) {
                    sum += c.getValue();
                }
                final Bill b = new Bill( request.getPatient(), l, sum, BillStatus.UNPAID, request.getDate() );
                billservice.save( b );
            }
