...
public class APIUserTest {
    ...
    public void testCreateUsers () throws Exception {
        ...
        //test Creating billingstaff
        final UserForm u4 = new UserForm( USER_4, PW, Role.ROLE_BILLINGSTAFF, 1 );
        
        mvc.perform( MockMvcRequestBuilders.post( "/api/v1/users" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( u4 ) ) ).andExpect( MockMvcResultMatchers.status().isOk() );
        
        final User billingstaff = service.findByName( USER_4 );
        Assert.assertNotNull( "The created user should be retrievable from the database", billingstaff );

        Assert.assertEquals( "The retrieved user should be a Personnel", Personnel.class, billingstaff.getClass() );

        Assert.assertEquals( "The retrieved user should have 1 role", 1, billingstaff.getRoles().size() );
